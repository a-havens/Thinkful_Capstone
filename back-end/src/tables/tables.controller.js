...
function hasOnlyValidTableProperties(req, res, next) {
  const { data = {} } = req.body;

  const invalidProperties = Object.keys(data).filter(
    (field) => !TABLE_PROPERTIES.includes(field)
  );
...
function isCapacityValueValid(req, res, next) {
...
const hasReservationId = sharedUtils.hasRequiredProperties("reservation_id");
...
async function isUnoccupied(req, res, next) {
...
module.exports = {
  create: [
    hasOnlyValidTableProperties,
    hasRequiredProperties,
    isCapacityValueValid,
    isValidTableName,
